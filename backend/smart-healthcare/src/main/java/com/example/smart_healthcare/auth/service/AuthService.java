package com.example.smart_healthcare.auth.service;
...
public class AuthService {
    ...
    public AuthResult refresh(String refreshToken) {
        ...
        Member member = memberRepository.findById(memberId)
                .orElseThrow(() -> new AppException(AuthErrorCode.AUTH_401_005));
        List<String> roles = memberRoleRepository.findByMember(member).stream()
        ...
        return new AuthResult(
                new LoginResponse(pair.accessToken(), jwtProvider.accessExpireSec(), "Bearer"),
                pair.refreshToken()
        );
    }
