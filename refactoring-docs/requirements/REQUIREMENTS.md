...
## [AUTH-001] 회원가입
- 사용자는 이메일/비밀번호 기반으로 회원가입할 수 있어야 한다.
- 이메일은 시스템 내 유일해야 하며 중복 가입을 허용하지 않아야 한다.
- 비밀번호는 안전한 해시(예: BCrypt)로 저장되어야 하며 평문 저장을 금지해야 한다.
- 회원가입 완료 시 기본 권한은 일반 사용자(USER)로 부여되어야 한다.

## [AUTH-002] 로그인/인증 토큰 발급
- 사용자는 이메일/비밀번호로 로그인할 수 있어야 한다.
- 로그인 성공 시 Access Token과 Refresh Token을 발급해야 한다.
- Access Token 만료시간은 30분으로 설정해야 한다.
- Access Token에는 최소한 사용자 식별자(`member_id`)와 권한(`role`)이 포함되어야 한다.
- 로그인 실패(미존재 계정, 비밀번호 불일치)는 표준 오류코드로 응답해야 한다.

## [AUTH-003] 토큰 재발급/로그아웃
- 사용자는 유효한 Refresh Token으로 Access Token을 재발급 받을 수 있어야 한다.
- Refresh Token 만료시간은 1주일(7일)로 설정해야 한다.
- 로그아웃 시 서버는 해당 Refresh Token을 더 이상 사용 불가 상태로 처리해야 한다.
- 탈취 대응을 위해 Refresh Token 회전(Rotation) 정책을 적용할 수 있어야 한다.

## [AUTH-004] 인가(권한) 기본 정책
- 인증이 필요한 API는 비인증 요청을 거부해야 한다.
- 리소스 수정/삭제는 소유자 본인 또는 관리자만 허용해야 한다.
- 관리자 전용 기능은 관리자 권한(ADMIN)으로 제한해야 한다.
